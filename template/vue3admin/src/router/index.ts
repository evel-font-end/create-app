...
let meanOperation: any, whiteRouterList: any; // 用户权限 && 白名单

router.beforeEach((to, from, next) => {
  const userInfo = (store as any).state.user.username;
  ...
  if (userInfo && store.state.meanOperation.size === 0) {
    if (meanOperation && meanOperation.length > 0) {
      meanOperation.forEach((item: any) => {
  ...
  if (!userInfo && to.name !== 'login') {
    ...
    if (meanOperation && whiteRouterList) {
      ...
      if (whiteRouterList.some((name: any) => name === toName)) {
