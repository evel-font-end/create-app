...
export default {
    ...
    mutations: {
        // 设置登陆用户信息
        setUserInfo(state: { username: any; user: any; }, { username, user }: any) {
        ...
        // 退出登录
        outUserLogin(state: { username: string; user: string; appCodes: { appCodes: never[]; appCode: undefined; }; }) {
            state.username = ''
            state.user = ''
    ...
    actions: {
        // 登录
        userLoginTo({ commit }: any, { username, password, captcha }: any) {
            username = username.trim()
            return new Promise((resolve, reject) => {

                userLogin({ username, password, captcha }).then((res: any) => {
                    if (res.code === "200") {
                        console.log('res', res);
                        const data = res.data
