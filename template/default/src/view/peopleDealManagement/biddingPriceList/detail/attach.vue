...
import { initTransLicenseAttach } from '@/api/peopleDealManagement/biddingPriceList';
...
export default {
  ...
  data () {
    return {
      ...
      isDetail: ['detail'].includes(this.idata.configType),
      attachDataList: {},
      dialog: {
        importFile: false, // 上传
        title: '附件', // 弹窗标题
  ...
  watch: {
    show (val) {
      ...
      this.isDetail = ['detail'].includes(this.idata.configType);
    }
  },
  mounted () {
    this.$nextTick(() => {
      if (this.idata && this.idata.id) {
        this.reqAttach();
      }
    });
  },
  methods: {
    ...
    uploadSubmit () {
      const { row, configType } = this.editData;
      ...
      wfForm.append('serviceCode', 'TRANS_LICENSE');
    ...
    reqAttach () {
      initTransLicenseAttach(this.idata.id).then(res => {
        if (res.code === '200') {
