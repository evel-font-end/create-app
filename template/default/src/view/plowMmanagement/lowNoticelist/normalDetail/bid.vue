        ...
        >
          <template slot-scope="scope" slot="operator">
            <el-button
              ...
              @click="handleAddBidClick('edit', { row: scope.row})"
            >
            </el-button>
          </template>
        </page-table>
...
export default {
  ...
  methods: {
    ...
    handleAddBidClick (configType = 'add', parames = null) {
    ...
    getTargetList (targetId) {
      ...
      return getPlowTarget(param).then(res => {
        const { data, total } = res.data;
        this.page.total = total;
        return data[0];
      });
...
      }, 10);
    ...
    // 获取交易类型下拉框数据
    _getTransactionType () {
      getPlowTransactionType().then(res => {
        if (res.code === '200') {
