...
export default {
  ...
  data () {
    ...
    const validateBeginApply = (rule, value, callback) => {
      ...
      if(value === ''){
        ...
        if(((this.data.configType === 'infoTrans' && curr_btnStatusData.isRestart === 1) || this.data.configType === 'add' || this.data.configType === 'edit') && this.isUpdateDate) {
          const startDate = Date.parse(curr_formData.beginNoticeTime.split(" ")[0]);
    ...
    const validateEndApply = (rule, value, callback) => {
      ...
      if(value === ''){
        ...
        if(((this.data.configType === 'infoTrans' && curr_btnStatusData.isRestart === 1) || this.data.configType === 'add' || this.data.configType === 'edit') && this.isUpdateDate) {
          const startDate = Date.parse(curr_formData.endNoticeTime.split(" ")[0]);
          const endDate = Date.parse(value.split(" ")[0]);
          if(startDate < endDate){
    ...
    const validateEndList = (rule, value, callback) => {
      ...
      if(value === ''){
        ...
        if(((this.data.configType === 'infoTrans' && curr_btnStatusData.isRestart === 1) || this.data.configType === 'add' || this.data.configType === 'edit') && this.isUpdateDate) {
          const startDate = Date.parse(curr_formData.beginListTime.split(" ")[0]);
          const endDate = Date.parse(value.split(" ")[0]);
      ...
      if(this.data.configType === 'add'){
        ...
        if(value === ''){
          ...
          if(((this.data.configType === 'infoTrans' && curr_btnStatusData.isRestart === 1) || this.data.configType === 'add' || this.data.configType === 'edit') && this.isUpdateDate) {
            //为拍卖时,不为挂牌时--1天
            //拿到年月日
            const startDate = Date.parse(curr_formData.endApplyTime.split(" ")[0]);
            const endDate = Date.parse(value.split(" ")[0]);
            if(endDate < startDate){
    ...
    return {
      ...
      formRules: {
        ...
        beginPrice: [{ required: true, message: '请输入起始价(万元)', trigger: 'blur' }],
        earnestMoney: [{ required: true, message: '0.00', trigger: 'blur' }],
        // hasReservePrice: 
        ...
        // transTrustDto: [{ required: false, message: '请选择委托人', trigger: 'blur' }],
        priceStep: [{ required: true, message: '请输入递减幅度(万元)', trigger: 'blur' }],
        reservePrice: [{ required: true, message: '请输入最低限价(万元)', trigger: 'change' }],
        isAfterCheck: [{ required: true, message: '请选择资格审核', trigger: 'blur' }],
        finalPrice: [{ required: false, message: '请输入最高限价(万元)', trigger: 'blur' }]
      },
      Options: {
        tableData: []
      },
  ...
  watch: {
    ...
    form: {
      deep: true,
      handler (now, old) {
        this.layout[1].formItems[3].inputShow = (now.hasReservePrice === 1);
      }
    }
  },
  methods: {
    ...
    isDisabled () {
      ...
      if (this.dictionaryNoticeCode && !this.canGetCode) return;
    ...
    reqDetail () {
      const { row } = this.data;
      if (row && row.id) {
        getPlowTargetDel(row.id).then(res => {
          if (res.code === '200') {
            this.form = {
              ...this.form,
              ...res.data,
              priceStep: Math.abs(res.data.priceStep)
            };
    ...
    submitForm () {
      ...
      this.$refs.supplyFormItem.$refs.formData.validate(valid => {
        ...
        param.id = this.data.configType === 'add' ? this.initId : this.form.id;
        param.operateFlag = this.data.configType;
