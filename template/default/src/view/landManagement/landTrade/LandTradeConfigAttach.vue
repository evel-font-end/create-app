      ...
      <collapse @item-click="itemClick" accordion>
        ...
        >
          ...
          <flex justify="flex-end">
            <button class="upload-button" v-if="!(isDetail) || item.isUpload === 1" @click.prevent.stop="() => handleUpdate(item.id)">
...
import { landTargetAttach } from '@/api'
import {
  transGoodsAttach,
  attachList,
...
export default {
  ...
  methods: {
    ...
    reqAttach () {
      ...
      landTargetAttach(param.goodsId).then(res => {
        if (res.code === '200') {
