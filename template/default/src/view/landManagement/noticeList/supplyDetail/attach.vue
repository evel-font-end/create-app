...
export default {
  ...
  watch: {
    ...
    data (val) {
      ...
      this.isAdd = val.configType === 'add';
      this.isDetail = ['detail'].includes(val.configType);
    }
  },
  mounted () {
    this.$nextTick(() => {
      if (this.editData.row && this.editData.row.id) {
        this.reqAttach();
      }
    });
  },
  methods: {
    ...
    reqAttach () {
      const { initId, isAdd, goodsId } = this;
      const paramId = isAdd ? initId : goodsId;
      getSuppleNoticeAttach(paramId).then(res => {
        if (res.code === '200') {
