...
<template>
  <div id='noticelist' >
        ...
        >
          ...
          <flex>
            <el-button 
              ...
              @click.prevent.stop="jump(scope.row.noticeType === 0 ? 'landNormal' : 'landSupply', 'detail', {row: scope.row})"
              v-if="$_hasOperation($options.name,'examine')">
            </el-button>
            <el-button 
              icon="iconfont icon-xiugaii" type="text" size="small" title="编辑"
              @click.prevent.stop="jump(scope.row.noticeType === 0 ? 'landNormal' : 'landSupply', 'edit', {row: scope.row})"
            ...
            <el-button 
              ...
              @click.prevent.stop="jump('landSupply', 'add', {row: scope.row})"
    ...
    <landNormal :show="editshow.landNormal" :data="editdata" @cancel="closeEdit"></landNormal>
    <landSupply :show="editshow.landSupply" :data="editdata" @cancel="closeEdit"></landSupply>
...
export default {
    ...
    "$route.query": {
      ...
      handler: function(to, from) {
        ...
        if (to.hasOwnProperty("noticeId") && this.common[name]) {
          ...
          setTimeout(() => {
            this.jump(to.noticeType === 0 ? 'landNormal' : 'landSupply', 'detail', {row: { ...to, id: to.noticeId }})
          }, 500);
        }
      }
    }
  ...
  methods: {
    ...
    getStatus () {
      getTransNoticeStatus().then(res => {
        if (res.code === '200') {
          ...
          this.$set(this.searchItems[2].props, 'options', option);
          this.$forceUpdate();
        }
      })
    },
    ...
    getDataAllList () {
      ...
      getTransNoticeList(data).then(res => {
        if (res.code === '200') {
    ...
    handleRelease (item) {
      publishTransNotice(item.id).then(res => {
        if (res.code === '200') {
          this.$notify({
            title: '成功',
            message: '发布成功',
    ...
    jump (path, configType, parames) {
      this.editdata = configType === 'add' && path !== 'landSupply' ? { configType } : { configType, ...parames };
