  ...
  <div class="container">
    ...
    >
      <template slot-scope="scope" slot="operator">
        <el-button type="text" size="small" title="详情" icon="iconfont icon-feeds-fill" @click="toPage(scope.row.id,'detail')" v-if="$_hasOperation($options.name,'examine')"></el-button>
...
export default {
  ...
  data () {
    return {
      tableCols: [
        { label: '宗地编号', prop: 'no', minWidth: 300 },
  ...
  methods: {
    ...
    toPage (id, type) {
      this.editdata = type === 'add' ? { type: type } : { type: type, id: id };
      this.editshow = true;
    },
    // closeEdit () {
    //   console.log('=3====')
    ...
    getList () {
      ...
      getTransTargetList(param).then(res => {
