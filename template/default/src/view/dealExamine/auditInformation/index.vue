      ...
      <div class="Modeltable">
        ...
        >
          <template slot-scope="scope" slot="operator">
            <el-button title="详情" icon="iconfont icon-feeds-fill" type="text" size="small" @click.prevent.stop="jump(scope.row)" v-if="$_hasOperation($options.name,'examine')"></el-button>
          </template>
        </page-table>
      </div>
    </div>
...
import { getApproveRecordList, getApproveLinkedBusinessType, getApproveStatus } from '@/api';
export default {
  ...
  data () {
    return {
      ...
      tableCols: [
        ...
        { label: '当前状态', prop: 'status', minWidth: '80px' },
        { label: '当前处理人', prop: 'curApproveUserRealName', minWidth: 100 },
        { label: '当前环节', prop: 'arriveNodeName', minWidth: 130 },
  ...
  methods: {
    ...
    getDataAllList () {
      ...
      getApproveRecordList(data).then(res => {
        if (res.code === '200') {
          const { data, total } = res.data;
          this.tableData = data;
          this.page.total = total;
          this.setRecord({total})
        }
      });
    },
    getSearchOptionList () {
