            ...
            <el-row :gutter="5">
              ...
              <el-col :span="12">
                <el-form-item :label="detailData.status" class="valueLine">
                  <span>{{ detailData.reservePrice }}万元</span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="5">
...
import { getHistoryList } from "@/api/dealManagement/plowDealManagement/plowLowPriceMangeList";
export default {
  name: "lowPriceDetail",
  components: {
    Wrapper,
  ...
  data() {
    return {
      ...
      tableCols: [
        // { label: "原底价(万元)", prop: "oldValue", minWidth: 200, align: 'right' },
        { label: "底价保护价(万元)", prop: "newValue", minWidth: 200, align: 'right' },
  ...
  watch: {
    show(val) {
      ...
      this.editshow = val;
      this.detailData.status = "底价：";
      this.detailData.reservePrice = this.data.row.reservePriceText;
    }
  },
  methods: {
    // 打开弹窗
