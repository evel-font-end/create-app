...
import AttachModal from './licenseAttach';
import { getTransTargetInfo,getBidderTradeTargetDtl, getTransBook } from "@/api/dealManagement/updatedealList";
import {
  nodeListByLicenceId,
  getApproveHistory
} from '@/api';
...
export default {
  ...
  watch: {
    data: {
      ...
      handler(val) {
        ...
        this.edit = false;
         if (val && this.dialogModel !== undefined && !this.dialogModel) {
          this.openedit()
        }
      }
    },
  ...
  created() {
    ...
    this.edit = false;
    if (this.data && this.dialogModel !== undefined && !this.dialogModel) {
      this.openedit()
    }
  ...
  methods: {
    ...
    _getTransTargetInfo() {
      let id = this.curr_id;
      getTransTargetInfo(id).then(res => {
          if (res.code === "200") {
            this.pageInfo = {  
              name: res.data.name,
    ...
    handleDownCJComfirBook () {
      ...
      if (this.data && this.dialogModel !== undefined && !this.dialogModel) {
        getTransBook(this.curr_licenseId,'DEAL_CONFIRM_BOOK').then(res => {
          if (res.code === '200') {
