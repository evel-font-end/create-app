      ...
      <div class="Modeltable">
        ...
        >
          <template slot-scope="scope" slot="operator">
            <!-- <span
              v-if="scope.row.canDetermine"
              @click="submitDetermineBtn(scope.row.licenseId, scope.row.id)"
              >摇号竞得</span
            > -->
            ...
            <el-button type="text" size="small" title="成交确认书提交" icon="iconfont icon-yubaofabu" @click="handleUpdate(scope.row.id, scope.row.licenseId)" v-if="scope.row.remark === '0' || scope.row.remark === '1'"></el-button> -->
...
// import attach from "./Edit/attach";
import edit from "./Edit/index";
import {
  handleDownloadCJTemplate
} from '@/util/util';
import {
  getCheckLandTargetList,
  getTransactionType,
  getBusinessType,
  getSubmitDetermine,
  getSubmitTrade,
...
export default {
  ...
  data() {
    return {
      ...
      tableCols: [
        ...
        { label: "交易结束时间", prop: "endTransTime", width: '170px' },
  ...
  methods: {
    ...
    getList() {
      ...
      getCheckLandTargetList(param).then(res => {
