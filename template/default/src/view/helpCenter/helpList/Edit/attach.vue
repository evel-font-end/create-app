...
import { transHelpAttach } from '@/api/helpCenter/helpList';
import {
  // transHelpAttach,
  attachList,
...
export default {
  ...
  methods: {
    ...
    uploadSubmit () {
      ...
      wfForm.append('serviceCode', 'TRANS_HELP');
    ...
    attachSubmit () {
      const { isAdd, targetId, id } = this;
      this.$refs.attachModel.validate(valid => {
        if (valid) {
          const request = {
            ...this.attachModel,
            refId: id,
            refTableName: 'TRANS_HELP'
          };
    ...
    getAttachList (id) {
      attachList(id).then(res => {
        if (res.code === '200') {
          ...
          this.isDetail = res.data.length>0 ? false : true;
        }
      });
    },
    reqAttach () {
      const { id } = this;
      transHelpAttach(id).then(res => {
        if (res.code === '200') {
          this.tableData = res.data;
          if(res.data[0].fileUploadInfoDto){
            this.isDetail = res.data[0].fileUploadInfoDto.length>0 ? false : true;
          }else{
            this.isDetail = true;
          }
